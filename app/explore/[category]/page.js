...
export default function CategoryPage({ params }) {
  // فك تشفير المتغير القادم من الرابط (مثل: ai, tech, crypto)
  const category = params.category || 'domains';

  // قوائم الكلمات الافتتاحية المخصصة للـ SEO لكي تزحف إليها محركات البحث
  const categoryKeywords = {
    ai: ['cyber', 'neural', 'mind', 'bot', 'deep', 'coreai', 'smartbot'],
    ...
    <main style={{ minHeight: '100vh', backgroundColor: '#0b0f19', color: '#fff', padding: '40px 20px', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ color: '#94a3b8', fontSize: '16px', marginBottom: '40px' }}>
          Explore premium verified web address ideas generated specifically for the <span style={{ color: '#2dd4bf', fontWeight: '600' }}>{category}</span> industry.
        </p>
        ...
        <div style={{ backgroundColor: '#131a26', border: '1px solid #1e293b', borderRadius: '12px', padding: '24px', textAlign: 'left' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {currentKeywords.map((keyword) => (
              <div key={keyword} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', backgroundColor: '#1a2333', borderRadius: '8px', border: '1px solid #1e293b' }}>
